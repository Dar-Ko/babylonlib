/*$Workfile: H:\_SrcPool\Cpp\KTaskIco.cpp$: implementation file
 ...
 */
CTaskBarIcon::CTaskBarIcon(UINT nID)
{
 ...
 */
UINT CTaskBarIcon::SetID(const UINT nID //new taskbar icon ID
                         )
{
...
void CTaskBarIcon::SetNotificationMsg(
                HWND  hNotifyWnd,     //receiving window
...
*/
BOOL CTaskBarIcon::SetIcon(UINT nID //= 0 ID of the taskbar icon to display or
                                    //remove (default)
                          )
{
if (nID != 0)
...
BOOL CTaskBarIcon::SetIcon(LPCTSTR lpszIconName,  //icon resource name
                           LPCSTR szTooltip       //= NULL tooltip string
                           )
{
return SetIcon(lpszIconName ?
               AfxGetApp()->LoadIcon(lpszIconName) : NULL, szTooltip);
}

BOOL CTaskBarIcon::SetIcon(HICON hicon, //handle of the taskbar icon to display
...
/*Loads a Windows predefined icon and displays it on the system's task bar.
  ...
  Parameters:
   ...
   szTooltip        pointer to the tooltip string or NULL
 */
BOOL CTaskBarIcon::SetIcon(const UINT nStandardIcoID,
                           LPCSTR szTooltip
                           )
{
LPCTSTR  lpIconName;
