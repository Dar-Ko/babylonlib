/*$RCSfile: statreg.h,v $: header file
  $Revision: 1.5 $ $Date: 2008/07/15 20:31:11 $
  $Author: ddarko $
...
#ifndef __STATREG_H
  #define __STATREG_H "ATL-3_00_PSDK_200603"
  ...
  public:
    ...
    BOOL AddString(LPCOLESTR lpsz)
    {
      ...
      while (*lpszT)
      {
        ...
        /*Fix: KB239909
          If the buffer contains DBCS characters, take into account
          the 2-byte characters.

          Rebuild the project with a Release MinDependency build configuration
          (If you do not build with this option, you would have to add
          _ATL_STATIC_REGISTRY to the preprocessor definitions of the project so
          ATL would add the registration code to your project. You would also
          have to remove the _ATL_DLL preprocessor option. You do not have to
          add _ATL_STATIC_REGISTRY if you are building with
          a Release MinDependency build configuration because it is already
          defined).
         */
        CharNext(lpszT);
      }
...
/*****************************************************************************
 * $Log: statreg.h,v $
 * Revision 1.5  2008/07/15 20:31:11  ddarko
 * Borland CC build and fixes
 *
 * Revision 1.1.2.3  2008/07/15 19:51:07  ddarko
 * Minor fixes
 *
 * Revision 1.1.2.2  2008/07/15 15:18:04  ddarko
