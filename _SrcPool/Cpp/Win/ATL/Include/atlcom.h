...
#ifndef __ATLCOM_H__
  #define __ATLCOM_H__ "ATL-3_00_PSDK_200603"
...
HRESULT WINAPI CComObject<Base>::CreateInstance(CComObject<Base>** pp)
{
  ...
  HRESULT hRes = E_OUTOFMEMORY;
  CComObject<Base>* p = NULL; KBCCDEF(p);
...
public:
                             ...
                             VARIANT* pvarResult)
  {
    ...
    //Fix: Add types in the helper
    VARTYPE* rgVarTypes[_ATL_MAX_VARTYPES]; //Fix
    VARTYPE** pVarTypes = info.nParams ? rgVarTypes : 0; //Fix

    for (int i=0; i<info.nParams; i++)
    {
      pVarArgs[i] = &pdispparams->rgvarg[info.nParams - i - 1];
      pVarTypes[i] = info.pVarTypes[info.nParams - i - 1]; //Fix
    }
 ...
 * Minor fixes
 *
 * Revision 1.1.2.4  2008/07/15 15:18:01  ddarko
