/*$Workfile: KDateCrl.cpp$: implementation file
  $Revision: 6$ $Date: 2003-01-14 16:49:50$
...
LPCTSTR CDateCtrl::DCTRLSTRCLASSNAME = _T("KDateEditCtrl");   //Window class name
const int GAP_X = 6; //Gap between columns in the Day Table
const int GAP_Y = 4; //Gap between rows in the Day Table

#ifdef _USE_DATECST
  extern "C" int g_iMonthLen[]; //month length table
#else
    //Month length table
  static const int g_iMonthLen[] =
...
//::SetFirstAndLastDay()-------------------------------------------------------
#include "KFileTme.h" //CFileTime class
...
m_nLastDay  = g_iMonthLen[m_iMonth-1];
...
m_spinMonth.SetRange(1, CST_MpYEAR);
...
void CDateCtrl::DoDataExchange(CDataExchange* pDX)
{
  ...
  DDV_MinMaxInt(pDX, m_iMonth,1, CST_MpYEAR);
 ...
 *  3    Biblioteka1.2         7/7/01 11:10:15 PM   Darko           $Revision: 6$
 *       inserted
