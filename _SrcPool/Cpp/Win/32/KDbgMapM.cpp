/*$Workfile: KDbgMapM.cpp$: implementation file
  $Revision: 8$ $Date: 2005-06-21 11:12:55$
...
/*Dumps the current mapping mode to the predefined CDumpContext object afxDump.
  ...
  - MM_HIMETRIC     Each logical unit is converted to 0.01 millimeter.
                    Positive xi s to the right; positive y is up.
  - MM_ISOTROPIC    Logical units are converted to arbitrary units with equally
                    scaled axes; that is, 1 unit along the x-axis is equal to 1
                    unit along the y-axis. Use the SetWindowExt and
                    SetViewportExt member functions to specify the desired units
                    and the orientation of the axes. GDI makes adjustments as
  ...
  - MM_TEXT         Each logical unit is converted to 1 device pixel.
